...
*   **Portfolio Dashboard Categorization (Dec 07)**:
    *   **Collapsible Fund Sections**: Grouped portfolio companies by normalized fund names (MVF1, MVF2, Motive AAV, Motive Create) with expandable/collapsible UI sections.
    *   **Navigation**: Added "Portfolio" to the global `CollapsibleMenu` and fixed z-index stacking context to ensure the menu remains accessible on all pages, including those with sticky headers.
