...
// NOTE: Embedding and AI enrichment are now handled in the parallel processing block
// by embed_interactions.ts and summarize_interactions.ts respectively.
// This keeps the sync fast and focused on data fetching only.
            ...
            do {
                ...
                for (let i = 0; i < currentBatch.length; i += CHUNK_SIZE) {
                    ...
                    await Promise.all(chunk.map(async (entry: any) => {
                        ...
                        // 1. NOTES (Store raw - enrichment happens in parallel block)
                        try {
                            ...
                            for (const note of notesResponse.notes) {
                                ...
                                // Store raw note - embedding will be generated by embed_interactions.ts
                                // AI enrichment will be done by summarize_interactions.ts
                                await this.upsertInteraction({
                                    ...
                                    content_preview: note.content.substring(0, 250),
                                    ...
                                    // embedding: null - will be filled by embed_interactions.ts
                                    // ai_* fields: null - will be filled by summarize_interactions.ts
                                });
                            }
                        } catch (e: any) {
                        ...
                        // 2. EMAILS (Store raw - embedding happens in parallel block)
                        try {
