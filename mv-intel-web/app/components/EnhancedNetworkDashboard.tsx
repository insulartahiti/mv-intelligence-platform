...
interface EnhancedIntroPathsResponse {
  success: boolean;
  targetEntity: TargetEntity;
  introductionPaths: IntroPath[];
  networkInsights: NetworkInsights | null;
  metadata: {
    totalPaths: number;
    maxPaths: number;
    minStrength: number;
...
export default function EnhancedNetworkDashboard() {
  ...
  const fetchIntroPaths = async (entityId: string) => {
    ...
    try {
      ...
      const data: EnhancedIntroPathsResponse = await response.json();
  ...
  const getConnectionTypeIcon = (type: string) => {
    switch (type) {
      case 'linkedin_industry_connection': return <Linkedin className="h-4 w-4" />;
      case 'portfolio_connection': return <Building className="h-4 w-4" />;
      case 'industry_connection': return <Network className="h-4 w-4" />;
