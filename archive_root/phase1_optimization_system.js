...
async function createOrUpdateEdge(sourceId, targetId, kind, strength = 0.5, metadata = {}) {
          ...
          for (const companyName of companyMentions) {
            ...
            if (companyEntity) {
              const success = await createOrUpdateEdge(
                ...
                0.8,
                { 
                  extraction_method: 'affinity_person_data',
...
function extractCompanyMentionsFromPersonData(enrichData, personName) {
  ...
  // Also search in web search data if available
  if (enrichData.web_search_data) {
    try {
      const webData = JSON.parse(enrichData.web_search_data);
      if (webData.results) {
        for (const result of webData.results) {
          ...
          const companyPatterns = [
            /at ([A-Z][a-zA-Z\\s&]+)/g,
            /works at ([A-Z][a-zA-Z\\s&]+)/g,
            /employed at ([A-Z][a-zA-Z\\s&]+)/g
          ];
    ...
    for (const company of portfolioCompanies || []) {
      ...
      if (fundId) {
        const success = await createOrUpdateEdge(
          ...
          0.9,
          { 
            extraction_method: 'affinity_status_field',
            fund_name: fundName,
            company_name: company.name
          }
        );
...
async function runPhase1Optimization() {
  console.log('🚀 Starting Phase 1 Optimization System...');
